...
import { getSubscription, updateSubscription } from '../utils/storage';
...
export function EditSubscriptionDialog({ subscriptionId, open, onOpenChange, onSuccess }: EditSubscriptionDialogProps) {
  ...
  const [indicator, setIndicator] = useState(subscription?.indicator || 'price');
  const [condition, setCondition] = useState(subscription?.condition || 'above');
  const [value, setValue] = useState(subscription?.value.toString() || '');
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            修改 {subscription.stockName} ({subscription.stockCode}) 的监控订阅
          </DialogDescription>
        </DialogHeader>
